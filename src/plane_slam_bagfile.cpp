...
int main(int argc, char** argv)
{
    ...
    if( argc < 2)
    {
        cerr << endl << "Usage: ./plane_slam_bagfile bagfile <skip> <duration> <paused>" << endl;
        return 1;
    }
    ...
    if( argc >= 3 )
        skip_time = atof(argv[2]);  // skip time
    if( argc >= 4 )
        duration = atof(argv[3]);   // play duration
    if( argc >= 5 )
    {
        std::string bool_str = argv[4];
    ...
    private_nh.param<string>("topic_camera_info", camera_info_topic, "/camera/depth/camera_info");
    //
    ...
    cout << GREEN << " Camera info topic: " << camera_info_topic << ", size = " << topic_sizes[2] << RESET << endl;
    cout << GREEN << " TF topic: " << tf_topic << ", size = " << topic_sizes[3] << RESET << endl;
    ...
    BOOST_FOREACH(rosbag::MessageInstance const m, view)
    {
        ...
        if( valid_depth && valid_rgb && valid_camera_info
